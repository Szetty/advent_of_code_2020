...
pub fn part1(inp: String) {
    let value = iterate_lines_and_apply_operations(
        ...
        |state, index, value| {
            state
                .memory
                .insert(index, apply_mask_on_value(value, state.mask));
        },
    );
    println!("{}", value);
}
...
fn iterate_lines_and_apply_operations(
    inp: String,
    mask_fn: fn(&mut State, [u8; 36]),
    mem_fn: fn(&mut State, i64, i64),
) -> i64 {
    let mut state = State {
        mask: [0u8; 36],
        memory: HashMap::new(),
...
fn apply_mask_on_index(index: i64, mask: [u8; 36]) -> Vec<i64> {
    ...
    bit_arrays
        .iter()
        .cloned()
        .map(transform_bit_array_to_number)
        .collect()
}
...
fn test_apply_mask_on_value() {
    assert_eq!(
        73,
        apply_mask_on_value(11, parse_mask("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X"))
    );
    assert_eq!(
        101,
        apply_mask_on_value(101, parse_mask("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X"))
    );
    assert_eq!(
        64,
        apply_mask_on_value(0, parse_mask("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X"))
    );
    assert_eq!(
        65082372640,
        apply_mask_on_value(
            122879146,
            parse_mask("11110X1XXX11001X01X00011001X00X00000")
        )
    );
    assert_eq!(
        0,
        apply_mask_on_value(1023, parse_mask("000000000000000000000000000000000000"))
    );
    assert_eq!(
        68719476735,
        apply_mask_on_value(
            68719476735,
            parse_mask("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
        )
    );
    assert_eq!(
        68719476735,
        apply_mask_on_value(
            19839428832,
            parse_mask("111111111111111111111111111111111111")
        )
    );
    assert_eq!(
        68719476735,
        apply_mask_on_value(0, parse_mask("111111111111111111111111111111111111"))
    );
}
...
fn test_apply_mask_on_index() {
    assert_eq!(
        vec![0, 1],
        apply_mask_on_index(0, parse_mask("00000000000000000000000000000000000X"))
    );
    assert_eq!(
        vec![26, 58, 27, 59],
        apply_mask_on_index(42, parse_mask("000000000000000000000000000000X1001X"))
    );
}
